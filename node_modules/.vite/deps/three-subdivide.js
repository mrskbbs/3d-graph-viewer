...
// node_modules/three-subdivide/src/LoopSubdivision.js
var POSITION_DECIMALS = 2;
var _average = new Vector3();
var _center = new Vector3();
var _midpoint = new Vector3();
var _normal = new Vector3();
var _temp = new Vector3();
var _vector0 = new Vector3();
var _vector1 = new Vector3();
var _vector2 = new Vector3();
var _vec0to1 = new Vector3();
var _vec1to2 = new Vector3();
var _vec2to0 = new Vector3();
var _position = [
  new Vector3(),
  new Vector3(),
  new Vector3()
...
var LoopSubdivision = class _LoopSubdivision {
  /////////////////////////////////////////////////////////////////////////////////////
  /////   Modify
  ////////////////////
  /**
  ...
  static modify(bufferGeometry, iterations = 1, params = {}) {
    ...
    if (typeof params !== "object")
      params = {};
    if (params.split === void 0)
      params.split = true;
    if (params.uvSmooth === void 0)
      params.uvSmooth = false;
    if (params.preserveEdges === void 0)
      params.preserveEdges = false;
    if (params.flatOnly === void 0)
      params.flatOnly = false;
    if (params.maxTriangles === void 0)
      params.maxTriangles = Infinity;
    if (params.weight === void 0)
      params.weight = 1;
    if (isNaN(params.weight) || !isFinite(params.weight))
      params.weight = 1;
    ...
    if (params.split) {
      const splitGeometry = _LoopSubdivision.edgeSplit(modifiedGeometry);
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      if (currentTriangles < params.maxTriangles) {
        ...
        if (params.flatOnly) {
          subdividedGeometry = _LoopSubdivision.flat(modifiedGeometry, params);
        } else {
          subdividedGeometry = _LoopSubdivision.smooth(modifiedGeometry, params);
        }
  ...
  static edgeSplit(geometry) {
    ...
    function splitAttribute(attribute, attributeName, morph = false) {
      ...
      let groupStart = void 0, groupMaterial = void 0;
      let index = 0;
      let skipped = 0;
      ...
      for (let i = 0; i < vertexCount; i += 3) {
        ...
        if (sharedCount === 0) {
          ...
          if ((length0to1 > length1to2 || edgeCount1to2 <= 1) && (length0to1 > length2to0 || edgeCount2to0 <= 1) && edgeCount0to1 > 1) {
            _center.copy(_vector0).add(_vector1).divideScalar(2);
            if (edgeCount2to0 > 1) {
              _midpoint.copy(_vector2).add(_vector0).divideScalar(2);
            ...
            if (edgeCount1to2 > 1) {
              _midpoint.copy(_vector1).add(_vector2).divideScalar(2);
            ...
            _center.copy(_vector1).add(_vector2).divideScalar(2);
            if (edgeCount0to1 > 1) {
              _midpoint.copy(_vector0).add(_vector1).divideScalar(2);
            ...
            _center.copy(_vector2).add(_vector0).divideScalar(2);
            if (edgeCount1to2 > 1) {
              _midpoint.copy(_vector1).add(_vector2).divideScalar(2);
        ...
        if (processGroups) {
          existing.groups.forEach((group) => {
            if (group.start === i - skipped) {
              if (groupStart !== void 0 && groupMaterial !== void 0) {
      ...
      if (processGroups && groupStart !== void 0 && groupMaterial !== void 0) {
  ...
  static flat(geometry, params = {}) {
    ...
    attributeList.forEach((attributeName) => {
      ...
      if (!attribute)
        return;
      loop.setAttribute(attributeName, _LoopSubdivision.flatAttribute(attribute, vertexCount, params));
    });
    const morphAttributes = existing.morphAttributes;
    for (const attributeName in morphAttributes) {
      const array = [];
      ...
      for (let i = 0, l = morphAttribute.length; i < l; i++) {
        if (morphAttribute[i].count !== vertexCount)
          continue;
        array.push(_LoopSubdivision.flatAttribute(morphAttribute[i], vertexCount, params));
      }
  ...
  static flatAttribute(attribute, vertexCount, params = {}) {
    ...
    for (let i = 0; i < vertexCount; i += 3) {
      ...
      _vector1.fromBufferAttribute(attribute, i + 1);
      _vector2.fromBufferAttribute(attribute, i + 2);
      _vec0to1.copy(_vector0).add(_vector1).divideScalar(2);
      _vec1to2.copy(_vector1).add(_vector2).divideScalar(2);
      _vec2to0.copy(_vector2).add(_vector0).divideScalar(2);
  ...
  static smooth(geometry, params = {}) {
    ...
    if (!verifyGeometry(geometry))
      return geometry;
    const existing = geometry.index !== null ? geometry.toNonIndexed() : geometry.clone();
    const flat = _LoopSubdivision.flat(existing, params);
    const loop = new BufferGeometry();
    const attributeList = gatherAttributes(existing);
    const vertexCount = existing.attributes.position.count;
    const posAttribute = existing.getAttribute("position");
    ...
    function addEdgePoint(posHash, edgeHash) {
      if (!existingEdges[posHash])
        existingEdges[posHash] = /* @__PURE__ */ new Set();
    ...
    for (let i = 0; i < vertexCount; i += 3) {
      ...
      _vec0to1.copy(_vertex[0]).add(_vertex[1]).divideScalar(2);
      _vec1to2.copy(_vertex[1]).add(_vertex[2]).divideScalar(2);
      _vec2to0.copy(_vertex[2]).add(_vertex[0]).divideScalar(2);
    ...
    attributeList.forEach((attributeName) => {
      ...
      if (existingAttribute === void 0 || flattenedAttribute === void 0)
        return;
    ...
    for (const attributeName in morphAttributes) {
      ...
      for (let i = 0, l = morphAttribute.length; i < l; i++) {
        ...
        const flattenedAttribute = _LoopSubdivision.flatAttribute(morphAttribute[i], morphAttribute[i].count, params);
        const floatArray = subdivideAttribute(attributeName, existingAttribute, flattenedAttribute);
...
function fuzzy(a, b, tolerance = 1e-5) {
...
three-subdivide/src/LoopSubdivision.js:
  (**
   ...
   *)
*/
//# sourceMappingURL=three-subdivide.js.map
